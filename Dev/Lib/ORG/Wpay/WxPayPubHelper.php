...
{	
	...
	function __construct($wxpay = array()) 
	{
		
		$this->appId = $wxpay['APPID'];
		$this->mchId = $wxpay['PARTNERID'];
		$this->key = $wxpay['APPKEY'];
		$this->appSecret = $wxpay['APPSERCERT'];
		//设置接口链接
	...
	function createXml()
	{
		try
		{
		    ...
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
		    $this->parameters["sign"] = $this->getSign($this->parameters);//签名
		    $ret =  $this->arrayToXml($this->parameters);
			return $ret;
