<?php
class ReplyAction extends UserAction
{

	protected function _initialize()
	{
		parent::_initialize();
		$this->function = 'pinglun';
		parent::checkOpenedFunction();
    }

	public function index()
	{
		$data=M('reply');
        $count      = $data->where(array('token'=>$this->token, 'status'=>array("neq", 0)))->count();
        $Page       = new Page($count,15);
        $show       = $Page->show();
        $list = $data->where(array('token'=>$this->token, 'status'=>array("neq", 0)))->order("id")->limit($Page->firstRow.','.$Page->listRows)->select();
        
        $this->assign('page',$show);
        $this->assign('list',$list);
        $this->display();
	}
	
	public function set()
    {
        $id    = $this->_get('id'); 
        $where          = array('token'=>$this->token,'id'=>$id, 'status' => 1);
        $reply       = M('Reply')->where($where)->find();
        if (empty($reply))
        {
            $this->error("没有微评论.您现在可以添加.",U('Reply/add'));
        }

        if(IS_POST)
        {
            $data['keyword']    = $this->_post('keyword');
            $data['title']      = $this->_post('title');
			$data['head_pic_url']     = $this->_post('head_pic_url'); 
			$data['msg_pic_url']     = $this->_post('msg_pic_url'); 
            $data['check']     = $this->_post('check'); 
			
			$admins = array();
            $admin_openids = $_POST['admin_openids'];
            for($i = 0; $i<count($admin_openids); $i ++) {
                if(!empty($admin_openids[$i])) {
                    array_push($admins, array($admin_openids[$i]));
                }
            }
            $data['admins'] = serialize($admins);
			
            $ret = M('Reply')->where($where)->save($data);

            if($ret)
            {
                $data1['module']   = 'reply';
                $data1['token']    = $this->token;
                $data1['function'] = $this->function;
				$data1['pid']       = $id;
                $da['keyword']     = $_POST['keyword'];
                M('Keyword')->where($data1)->save($da);
                $this->success('修改成功',U('Reply/index'));
            }
            else
            {
                $this->error('服务器繁忙,请稍后再试',U('Reply/index'));
            }
        } else { 
		    if(!empty($reply)) {
		        $admins = unserialize($reply['admins']);
                $this->assign('admins', $admins);
			}
            $this->assign('set',$reply);
            $this->display();  
        }
    }
    
    public function add()
    { 
        if (IS_POST)
        {
            $data['keyword']    = $this->_post('keyword');
            $data['title']      = $this->_post('title');
			$data['head_pic_url']     = $this->_post('head_pic_url'); 
			$data['msg_pic_url']     = $this->_post('msg_pic_url'); 
			$data['check']     = $this->_post('check'); 
			
			$admins = array();
            $admin_openids = $_POST['admin_openids'];
            for($i = 0; $i<count($admin_openids); $i ++) {
                if(!empty($admin_openids[$i])) {
                    array_push($admins, array($admin_openids[$i]));
                }
            }
            $data['admins'] = serialize($admins);
			
            $data['token']      = $this->token;
            $data['createtime']  = time();
			$data['status']  = 1;

            $merchant_id = M('reply')->data($data)->add();
            if($merchant_id)
            {
                $kwds_db = M('keyword');
                $kwd_data['uid']      = session('uid');
                $kwd_data['token']    = $this->token;
                $kwd_data['type']     = 1;
                $kwd_data['module']   = 'reply';
                $kwd_data['function'] = $this->function;
                $kwd_data['pid']      = $merchant_id;
                $kwd_data['keyword']  = $_POST['keyword']; 
                $kwds_db->add($kwd_data);
            }
            $this->success('修改成功',U('Reply/index'));
        }
        else
        {
            $this->display('set');
        }
    }
	
	public function del()
    {
        $id = $this->_get('id');

        if(IS_GET)
        {                              
            $where  = array('id'=>$id,'token'=>$this->token, 'status'=>1);
            $back=M('Reply')->where($where)->save(array('status'=>0));
            if($back==true)
            {
                M('Keyword')->where(array('pid'=>$id,'token'=>$this->token,'module'=>'reply', 'function'=> $this->function))->delete();
                $this->success('操作成功',U('Reply/index'));
            }
            else
            {
                $this->error('服务器繁忙,请稍后再试',U('Reply/index'));
            }
        }        
    }
	
	public function admin()
	{
	    $id = $this->_get('id'); 
		$data=M('reply_reply');
        $count      = $data->where(array('rid'=>$id, "token"=>$this->token))->count();
        $Page       = new Page($count,15);
        $show       = $Page->show();
        $list = $data->where(array('rid'=>$id, "token"=>$this->token/*, 'status'=>array("neq", 0)*/))->order("id desc")->limit($Page->firstRow.','.$Page->listRows)->select();
        
        $this->assign('page',$show);
        $this->assign('list',$list);
		$this->assign('rid',$id);
        $this->display();
	}
	
	public function onboard()
    {
        $id = $this->_post('id');
		
        $url = C('site_url').U('Reply/admin', array('id'=>$this->_post('rid')));
        $data['url'] = $url;
        if(IS_POST)
        {                              
            $back=M('reply_reply')->where(array('id'=>$id,'token'=>$this->token))->save(array('status'=>2));
            if($back==true)
            {
                $this->ajaxReturn($data, "已审核", 1);
            }
            else
            {
                $this->ajaxReturn("", "服务器繁忙,请稍后再试", 0);
            }
        }        
    }
	
	public function delreply()
    {
        $id = $this->_post('id');
		$url = C('site_url').U('Reply/admin', array('id'=>$this->_post('rid')));
        $data['url'] = $url;
        if(IS_POST)
        {                              
            $back=M('reply_reply')->where(array('id'=>$id,'token'=>$this->token))->save(array('status'=>0));
            if($back==true)
            {
                $this->ajaxReturn($data, "已屏蔽", 1);
            }
            else
            {
                $this->ajaxReturn("", "服务器繁忙,请稍后再试", 0);
            }
        }        
    }
}
?>