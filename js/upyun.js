...
function editWsClass(domid,domid2,cid){
	art.dialog.data('domid', domid);
	art.dialog.data('domid2', domid2);
	art.dialog.data('cid', cid);
	art.dialog.open('?g=User&m=WsClassify&a=editWsClass&id='+cid,{lock:true,title:'选择分类',width:600,height:500,yesText:'关闭',background: '#000',opacity: 0.45});
}


function editCompanyClass(domid,domid2,cid){
	art.dialog.data('domid', domid);
	art.dialog.data('domid2', domid2);
	art.dialog.data('cid', cid);
	art.dialog.open('?g=User&m=WsCompanyClassify&a=editCompanyClass&id='+cid,{lock:true,title:'选择分类',width:600,height:500,yesText:'关闭',background: '#000',opacity: 0.45});
}

function editWscClass(domid,domid2,cid){
	art.dialog.data('domid', domid);
	art.dialog.data('domid2', domid2);
	art.dialog.data('cid', cid);
	art.dialog.open('?g=Wsc&m=WscClassify&a=editWscClass&id='+cid,{lock:true,title:'选择分类',width:600,height:500,yesText:'关闭',background: '#000',opacity: 0.45});
}