...
FCAPP.HOUSE.FULL3D = {
    ...
    init: function() {
        ...
        window.shareData.linkKeep = 'index.php/3d/gm/'+TOKEN+'/'+GALLERYID;
    ...
    loadData: function() {
        ...
        $.ajax({
            ...
            url: 'http://'+location.hostname+'/index.php/3d/gm/' + TOKEN + '/' + GALLERYID,
            dataType: 'jsonp',
            error: function() {
                FCAPP.Common.msg(true, {
                    msg: '无效的相册！'
        ...
        if (data.rooms) {
            ...
            for (var i = 0, il = rooms.length; i < il; i++) {
                ...
                if (rooms[i].full3d) {
                    ...
                    window.shareData.desc = '【360度全景】-' + rooms[i].desc + rooms[i].name;
                    window.shareData.descKeep = '【360度全景】-' + rooms[i].desc + rooms[i].name;
                    break;
                }
            }
            if (full3d.length > 0) {
